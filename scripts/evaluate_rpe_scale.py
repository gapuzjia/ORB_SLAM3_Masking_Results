...
def associate_timestamps(gt_stamps, est_stamps, max_diff=0.02):
    ...
    Args:
        gt_stamps: Ground truth timestamps
        est_stamps: Estimated trajectory timestamps
        max_diff: Maximum time difference for association (default: 0.02s = 20ms)
    
    Returns:
        List of (gt_timestamp, est_timestamp) tuples
    """
    matches = []
...
def compute_rpe(gt_trajectory, est_trajectory, max_time_diff=0.02):
    ...
    Args:
        gt_trajectory: Ground truth trajectory dict
        est_trajectory: Estimated trajectory dict
        max_time_diff: Maximum time difference for timestamp association
    
    Returns:
        trans_errors: List of translational errors (meters)
        rot_errors: List of rotational errors (degrees)
    """
    ...
    if len(matches) < 2:
        raise ValueError(f"Insufficient timestamp matches: {len(matches)}. "
                        f"Need at least 2 matches to compute RPE.")
...
def plot_errors(trans_errors, rot_errors, output_path):
    ...
    Args:
        ...
        rot_errors: List of rotational errors
    ...
    with open(csv_path, mode='a', newline='') as file:
        ...
        if not file_exists:
            writer.writerow(["run_id", "dataset", "trans_rmse", "trans_mean", "trans_median", 
                           "trans_std", "trans_max", "rot_rmse", "rot_mean", "rot_median",
                           "rot_std", "rot_max"])
        writer.writerow([run_id, dataset, rmse_trans, mean_trans, median_trans, std_trans, max_trans,
                        rmse_rot, mean_rot, median_rot, std_rot, max_rot])
...
def main():
    ...
    try:
        ...
        # Extract metadata for CSV - robust method
        ...
        # Remove common prefixes like "GT_"
        if dataset.startswith("GT_"):
            dataset = dataset[3:]
